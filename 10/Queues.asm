...
;     12/01/16	Dong Hyun Kim	 revised code and updated comments

$INCLUDE(Queues.INC)    ;add the include file with definitions and structure
$INCLUDE(General.INC)   ;add the general definitions
...
;                       maximum number of bytes that can be stored in the queue. 
...
; Limitations:			Length of the queue is fixed.
; Known Bugs:           None.
; Special Notes:        None.
;
...
;						12/01/16	Dong Hyun Kim		revised code and updated
;														comments
...
SizeByte:                                 ;size of element is BYTE_SIZE byte
        MOV     [SI].ele_size, BYTE_SIZE  ;set size of element to BYTE_SIZE byte
        JMP     EndQueueInit              ;done with initializing the queue

SizeWord:                                 ;size of element is WORD_SIZE byte
        MOV     [SI].ele_size, WORD_SIZE  ;set size of element to WORD_SIZE bytes
        ;JMP    EndQueueInit              ;done with initializing the queue
...
;
; Local Variables:		h (AX)      - stored index of head.
; Shared Variables:		None.
; Global Variables:		None.
;
; Input:				None.
...
;
; Local Variables:		t (AX)          - stored index of (tail + size).
; Shared Variables:		None.
; Global Variables:		None.
;
; Input:				None.
...
CheckIfFull:                            ;determine if queue is full
        ADD     AX, [SI].ele_size       ;add BYTE_SIZE to t if byte, WORD_SIZE
                                        ;to t if word
        AND     AX, MAX_LENGTH - 1      ;t (AX) = t (AX) mod QUE_LENGTH, since 
										;	QUE_LENGTH is the power of 2 this 
										;	operation is valid. (QUE_LENGTH = 
										;	MAX_LENGTH - 1)
...
CheckEleSize:                           ;determine size of element to be removed
        CMP     [SI].ele_size,WORD_SIZE ;check if size of element is WORD_SIZE 
                                        ;or WORD_SIZE
...
DequeueByte:                            ;prepare value to return (byte)
        MOV     AL, [SI].array[BX]      ;store value of head in v (AL)
        JMP     DequeueEle              ;now, remove element from queue

DequeueWord:                            ;prepare value to return (word)
        MOV     AX, WORD PTR [SI].array[BX]		;store value of head in v
...
; Return Value:			None.
;
; Local Variables:		t (BX)            - stored index of tail.
; Shared Variables:		None.
; Global Variables:		None.
;
; Input:				None.
...
StoreByte:                              ;prepare to store value (byte)
        MOV     [SI].array[BX], AL      ;set v (AL) as the value of tail
        JMP     EnqueueEle              ;now, take care of index of tail

StoreWord:                              ;prepare to store value (word)
        MOV     WORD PTR [SI].array[BX], AX		;store v in value of the tail
        ;JMP    EnqueueEle              ;now, take care of index of tail
