...
CheckIfNull:                            ;checks to see if there is ASCII_NULL
        ...
        SAR     BX, 1                   ;currently, the index is for a word array
                                        ...
                                        ;   BYTES_PER_WORD every loop. Since we
...
SetPattern:                             ;sets the segment pattern for the digits
        ...
        ADD     BX, BYTES_PER_WORD      ;stay in loop by incrementing i
...
ClearRemainderLoop:                     ;loop to clear any data after the ASCII_NULL
        ...
        MOV     MuxBuffer[BX], 0        ;if i < BUF_LENGTH, set the element in 
                                        ;   the MuxBuffer to 0
        ADD     BX, BYTES_PER_WORD      ;stay in loop by incrementing the word
                                        ;   index properly
...
ClearMuxBuf:							;sets the value of current digit to 0
		MOV		MuxBuffer[SI], 0		;initializes the value of current digit
										;	by setting it to 0
		ADD		SI, BYTES_PER_WORD  	;move on to the next element of MuxBuffer
...
;                       MuxBuffer by incrementing MuxIndex by BYTES_PER_WORD. It
;                       will wrap around MuxBuffer apporpriately utilizing the
...
CheckIndex:								;Wrap around MuxBuffer if index is greater
                                        ;   than BUF_LENGTH
        ADD     BX, BYTES_PER_WORD		;Move on to the next index of digit
