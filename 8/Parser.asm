...
; Description: This file contains sixteen public functions and three private tables
;			   that will be used for the serial parcing routine of the RoboTrike.
...
;			   state machine. The first two functions initialize the shared 
;			   variables and return the token type and value for the parsed
;			   character. The remaining twelve functions are utilized as the
;			   aciton routines of the state machine. 
;
...
;	SetAbsSpeed:		Sets the asbolute speed of the RoboTrike.
...
; local include files
$INCLUDE(Parser.INC)			;contains definitions and addresses for parcing
...
; Arguments:			c 	        (AL) - The passed character to process as a 
;								           serial line.
; Return Value:			ParserError (AX) - status of the parsing operation.
;										   NO_ERROR if there was no error, 
;										   PARSING_ERROR if otherwise.
;
...
CallSetAbsSpeed:                                ;call the appropriate function
...
; Operation:			The action routine initally calls GetMotorSpeed to obtain
...
CallSetRelSpeed:                                ;call the appropriate function
...
;                       since we do not want to change the speed setting.
;
; Arguments:			None.
; Return Value:			None.
;
...
;
; Input:				<return> from the serial line.
; Output:				Change of direction of movment on the RoboTrike.
;
; Global Variables:		None.
;
; Input:				None.
...
CallSetDirection:                               ;call the appropriate function
...
;                       onff Otherwise, it will be turned on. 
;
; Arguments:			None.
; Return Value:			None.
;
